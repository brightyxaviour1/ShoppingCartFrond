...
public class CartController {
	...
	@RequestMapping(value = "/myCart1", method = RequestMethod.GET)
	public String myCart1(Model model1) {
		model1.addAttribute("product", new Product());
		model1.addAttribute("productList", this.productDAO.list());
		model1.addAttribute("cartSize", this.cartDAO.list().size());
	...
	public String addToCart(@PathVariable("id") String id,HttpSession session){
	 ...
	 cart.setUser(userDAO.get(loggedInUser_ID));   //  id should keep session and use the same id
	 cart.setStatus('N');  // 
